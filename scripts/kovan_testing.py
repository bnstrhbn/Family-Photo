...
def deploy():
    ...
    creating_tx2 = parent_nft.makeNewGroup({"from": account})
    creating_tx2.wait(1)
    print("New parent token has been created! (2)")
    return parent_nft
...
def main():
    ...
    child_nft = deploy_child(
        "https://ipfs.io/ipfs/QmWx8iszzn2ffb3uDvvojnDbdfMg7qZLxheeprENGnsszP?filename=alienpunk.png"
    )

    child_nft2 = deploy_child(
        "https://ipfs.io/ipfs/QmNmkSyvjvMMWNJm86PkUsZk3pxYJsk5mW89qgNRZL8Tnn?filename=punk3840.png"
    )
    child_nft3 = deploy_child(
        "https://ipfs.io/ipfs/QmQ9jJ3AjjmPPvNGwBUCbrLvr85ZeBHJ6N4Rv6EaUW2wGL?filename=test3impfs.png"
    )
    child_nft4 = deploy_child(
        "https://ipfs.io/ipfs/QmeM624ZA1gNVP4JKgyar7ABjxGHhvDdrzsrgw5cvJYnR3?filename=punk8111.png"
    )
    child_nft5 = deploy_child(
    ...
    createChild(child_nft4, accSwitch)
    createChild(child_nft5, accSwitch)

    # 1
    open_parent_and_add(parent_nft, 0, child_nft3, 0)
    open_parent_and_add(parent_nft, 0, child_nft, 0)
    open_parent_and_add(parent_nft, 0, child_nft, 1)
    open_parent_and_add(parent_nft, 0, child_nft, 2, accSwitch)
    open_parent_and_add(parent_nft, 0, child_nft2, 0, accSwitch)
    open_parent_and_add(parent_nft, 0, child_nft2, 1, accSwitch)
    open_parent_and_add(parent_nft, 0, child_nft4, 0, accSwitch)
    open_parent_and_add(parent_nft, 0, child_nft5, 0, accSwitch)
    # 2
    open_parent_and_add(parent_nft, 1, child_nft3, 0)
    open_parent_and_add(parent_nft, 1, child_nft, 0)
    open_parent_and_add(parent_nft, 1, child_nft, 1)
    open_parent_and_add(parent_nft, 1, child_nft, 2, accSwitch)
    open_parent_and_add(parent_nft, 1, child_nft2, 0, accSwitch)
    ...
    # TODO - add error handling in EA in case the return is null (something about ipfs/myEA sometimes returns null responses?)
